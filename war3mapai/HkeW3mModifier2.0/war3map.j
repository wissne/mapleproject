...
function InitTrig_BlockUseItem takes nothing returns nothing
    set gg_trg_BlockUseItem=CreateTrigger()
    call TriggerRegisterAnyUnitEventBJ(gg_trg_BlockUseItem,EVENT_PLAYER_UNIT_ISSUED_ORDER)
    call TriggerRegisterAnyUnitEventBJ(gg_trg_BlockUseItem,EVENT_PLAYER_UNIT_ISSUED_POINT_ORDER)
    call TriggerRegisterAnyUnitEventBJ(gg_trg_BlockUseItem,EVENT_PLAYER_UNIT_ISSUED_TARGET_ORDER)
    call TriggerAddAction(gg_trg_BlockUseItem,function UnitBlockItemUsage_Sub)
endfunction
function UnitBlockAbilityUsage_Sub takes nothing returns nothing
    if GetUnitUserData(GetSpellAbilityUnit())!=11 then
    endif
    call DisplayToAll("id:"+I2S(GetSpellAbilityId()))
    call DisplayToAll("name:"+GetObjectName(GetSpellAbilityId()))
...
function InitTrig_BlockUseAbility takes nothing returns nothing
    set gg_trg_BlockUseAbility=CreateTrigger()
    call TriggerRegisterAnyUnitEventBJ(gg_trg_BlockUseAbility,EVENT_PLAYER_UNIT_SPELL_CAST)
    call TriggerAddAction(gg_trg_BlockUseAbility,function UnitBlockAbilityUsage_Sub)
endfunction
function UnitBlockAbilityUsage_Sub2 takes nothing returns nothing
    if GetUnitUserData(GetTriggerUnit())!=11 then
        return
    endif
    call PauseUnit(GetTriggerUnit(),true)
    call IssueImmediateOrder(GetTriggerUnit(),"stop")
    call DisplayToAll(AbilityId2String(GetSpellAbilityId()))
    call PauseUnit(GetTriggerUnit(),false)
endfunction
function InitTrig_BlockUseAbility2 takes nothing returns nothing
    set gg_trg_BlockUseAbility2=CreateTrigger()
    call TriggerRegisterAnyUnitEventBJ(gg_trg_BlockUseAbility2,EVENT_PLAYER_UNIT_SPELL_CHANNEL)
    call TriggerAddAction(gg_trg_BlockUseAbility2,function UnitBlockAbilityUsage_Sub2)
endfunction
function getUnitByid takes integer iunitid,location loc,real radius returns unit
    ...
    set g=null
    if(ru==null)then
        call DisplayToAll("unit not found")
    else
        call DisplayToAll("unit  found")
    endif
    return ru
endfunction
function HeroBuyItem takes unit whichHero,integer shopid,integer Itemid returns nothing
    local boolean bbuyed=false
    local unit shop
    set shop=getUnitByid(shopid,GetUnitLoc(whichHero),600)
    if shop!=null then
        set bbuyed=IssueNeutralImmediateOrderById(Player(1),shop,Itemid)
    else
        call DisplayToAll("no Itemshop found")
    endif
    if bbuyed then
        call DisplayToAll("buyed                 Yes ")
    else
        call DisplayToAll("Not  没有买到     buyed ")
    endif
endfunction
function condition_is_unitid takes nothing returns boolean
    return GetUnitTypeId(GetFilterUnit())=='n00R'
endfunction
function Trig_showHeroLoc_Func001001002 takes nothing returns boolean
    return(IsUnitType(GetFilterUnit(),UNIT_TYPE_HERO)==true)
endfunction
function Trig_showHeroLoc_Actions takes nothing returns nothing
    local unit udg_tmpUnit
    local string udg_showStr
    local unit u
    set udg_tmpUnit=FirstOfGroup(GetUnitsOfPlayerMatching(Player(1),Condition(function Trig_showHeroLoc_Func001001002)))
    call SetUnitInvulnerable(udg_tmpUnit,true)
    set udg_showStr="X:"+R2S(GetUnitX(udg_tmpUnit))+" Y:"+R2S(GetUnitY(udg_tmpUnit))
    call CreateTextTagLocBJ(udg_showStr,GetUnitLoc(udg_tmpUnit),0,20.00,100,100,100,0)
    call SetTextTagPermanent(GetLastCreatedTextTag(),false)
    call SetTextTagLifespan(GetLastCreatedTextTag(),5)
    call SetTextTagFadepoint(GetLastCreatedTextTag(),5.00)
    set u=FirstOfGroup(GetUnitsInRangeOfLocMatching(600.0,GetUnitLoc(udg_tmpUnit),Condition(function condition_is_unitid)))
endfunction
function zsMeleeStartAI takes nothing returns nothing
    local integer index
        ...
        if((GetPlayerSlotState(indexPlayer)==PLAYER_SLOT_STATE_PLAYING)and(index!=6))then
            ...
            if(GetPlayerController(indexPlayer)==MAP_CONTROL_COMPUTER)then
                if GetAIDifficulty(indexPlayer)==AI_DIFFICULTY_NORMAL then
                    ...
                    call DisplayToAll("给普通电脑玩家"+I2S(index)+" 增加 200 个金币,5个木")
                endif
                if GetAIDifficulty(indexPlayer)==AI_DIFFICULTY_INSANE then
                    ...
                    call DisplayToAll("给疯狂电脑玩家"+I2S(index)+" 增加 250 个金币,10个木")
                endif
            endif
        endif
        set index=index+1
...
function InitTrig_showHeroLoc takes nothing returns nothing
    local unit tmp
...
function Trig_Leaves_Actions takes nothing returns nothing
    ...
    call StartCampaignAI(GetTriggerPlayer(),"zhensan.ai")
    call DisplayToAll("为离线玩家："+GetPlayerName(GetTriggerPlayer())+"开启AI")
endfunction
...
//    call TriggerAddAction(gg_trg_No_Double_Hero,function Trig_No_Double_Hero_Actions)
    ...
    call TriggerRegisterTimerEventPeriodic(gg_trg_SU_Add_Gold,1.00)
    call TriggerAddAction(gg_trg_SU_Add_Gold,function Trig_SU_Add_Gold_Actions)
    set gg_trg_WEI_Add_Gold=CreateTrigger()
    call TriggerRegisterTimerEventPeriodic(gg_trg_WEI_Add_Gold,1.00)
